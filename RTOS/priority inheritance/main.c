...
static void prvNewPrintString1( const portCHAR *pcString1 );
//function of medium prioriy task//
...
int main( void )
{
	...
	if( xMutex != NULL )
	{
		...
		xTaskCreate( prvPrintTask1, "Print3", 240, "Task 3 ///////////////////////////////////////////\n", 3, &xTask3Handle );
	...
	xSemaphoreTake( xMutex, portMAX_DELAY );
	{
		j=0;
		 while(j<(3180*600)){
		  ...
		  //lower priority task will be blocked on trying to take this resource//
...
static void prvNewPrintString1( const portCHAR *pcString1 )
{
static char cBuffer1[ mainMAX_MSG_LEN ];
	...
	{
	  //higher priority task will be blocked on trying to take this resource//
		/* The following line will only execute once the semaphore has been
		successfully obtained - so standard out can be accessed freely. */
		sprintf( cBuffer1, "%s", pcString1 );
		printf( cBuffer1 );
	}
...
static void prvPrintTask( void *pvParameters )//lower priority//
{
...
unsigned portBASE_TYPE uxPriority;
	...
	
	{
		...
		vTaskPrioritySet( xTask3Handle,1 );
		/* Print out the string using the newly defined function. */
		prvNewPrintString( pcStringToPrint );
		vTaskPrioritySet( NULL ,1 );
		vTaskPrioritySet( xTask3Handle,3 );
...
static void prvPrintTask1( void *pvParameters )//higher priority//
{
	
char *pcStringToPrint1;

	/* Two instances of this task are created so the string the task will send
	to prvNewPrintString() is passed in the task parameter.  Cast this to the
	required type. */
	
	pcStringToPrint1 = ( char * ) pvParameters;

	for( ;; )
	{
		/* Print out the string using the newly defined function. */
			vTaskDelay( 500/ portTICK_RATE_MS );
		prvNewPrintString1( pcStringToPrint1 );
...
static void prvPrintTask2( void *pvParameters ){//higher priority//
