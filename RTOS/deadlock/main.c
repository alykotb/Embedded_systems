...
int main( void )
{
	...
	if( xMutex != NULL )
	{
		...
		xTaskCreate( prvPrintTask1, "Print2", 240, "Task 2 ------------------------------------------\n", 2, NULL );
	...
	xSemaphoreTake( xMutex, portMAX_DELAY );
	{
		j=0;
		 while(j<(3180*600)){
		  ...
		  prvNewPrintString1( pcString );//lower priority task will be blocked on trying to take this resource//
...
static char cBuffer1[ mainMAX_MSG_LEN ];
	...
	{
		prvNewPrintString( pcString1 );   //higher priority task will be blocked on trying to take this resource//
		/* The following line will only execute once the semaphore has been
		successfully obtained - so standard out can be accessed freely. */
...
static void prvPrintTask( void *pvParameters )//lowe priority//
{
