...
const Mcu_PerClockConfigType McuPerClockConfigData [NUM_OF_CAN_CONTROLLERS]= SYSCTL_PERIPH;/*shall obtained as extern from another
                                                                                               and pointed to it */
const Can_ConfigType CanContainer= UserCANCfg;
 ...
 {
     if(CanContainer.CanConfigSet.CanHardwareObject[index0].CanObjectType==transmit)
     {
         if(CanContainer.CanConfigSet.CanHardwareObject[index0].CanControllerRef==&CanContainer.CanConfigSet
                 .CanController[0])
         {
             for(index1=0U;index1<CanContainer.CanConfigSet.CanHardwareObject[index0].CanHwObjectCount;index1++)
             {
...
static inline const CanHardwareObject* Can_FindTxHoh(Can_HwHandleType hth)
{
    ...
    MAP = &CanContainer.CanConfigSet.CanHardwareObject[hth];
    return MAP;
...
tCANMsgObject CANMessage;
    ...
    {
        if(CanContainer.CanConfigSet.CanHardwareObject[index2].CanObjectType==receive)

        {
            if(CanContainer.CanConfigSet.CanHardwareObject[index2].CanControllerRef==&CanContainer.CanConfigSet
                            .CanController[0])
          {
            for(index3=0U;index3<CanContainer.CanConfigSet.CanHardwareObject[index2].CanHwObjectCount;index3++)
            {

              Message_Confirmation[index2][index3].mail_box = controller0_receiveObjects_counter;
              CANMessage.ui32MsgID=CanContainer.CanConfigSet.CanHardwareObject[index2].CanHwFilter.CanHwFilterCode;
              CANMessage.ui32MsgLen=8U;
              CANMessage.ui32MsgIDMask=CanContainer.CanConfigSet.CanHardwareObject[index2].CanHwFilter.CanHwFilterMask;
              ...
              CANMessageSet(CanContainer.CanConfigSet.CanHardwareObject[index2].CanControllerRef->CanControllerBaseAddress,
...
//       while(1)
//       {
    if(CanContainer.CanConfigSet.CanController[0].CanControllerActivation)
    {
          CAN0_IF1CMSK_R&=((uint32_t)(~(uint32_t)0x80U));
    }else{/*MISRA*/}
    if(CanContainer.CanConfigSet.CanController[1].CanControllerActivation)
       {
       ...
       for(index4=0U;index4< NUM_OF_HOH;index4++)
         {
               if(CanContainer.CanConfigSet.CanHardwareObject[index4].CanObjectType==transmit)
               {
                   if(CanContainer.CanConfigSet.CanHardwareObject[index4].CanControllerRef== &CanContainer.CanConfigSet
                           .CanController[0])
                   {
#if (CanTxProcessing0==POLLING)


                     for(index5=0U;index5<CanContainer.CanConfigSet.CanHardwareObject[index4].CanHwObjectCount;index5++)
                                   {
         ...
         for(index6=0U; index6< NUM_OF_HOH;index6++)
           {
                 ...
                 if(CanContainer.CanConfigSet.CanHardwareObject[index6].CanObjectType==receive)
                 {

                     if(CanContainer.CanConfigSet.CanHardwareObject[index6].CanControllerRef==&CanContainer
                             .CanConfigSet.CanController[0])
                       {
#if (CanRxProcessing0==POLLING)

                    for(index7=0U;index7<CanContainer.CanConfigSet.CanHardwareObject[index6].CanHwObjectCount;index7++)
                        {
                          ...
                          POLLING_Register|=0x7FFFU;
                            ...
                            /*[SWS_Can_00279] On L-PDU reception, the Can module shall call the RX indication callback
                             ...
                             and the Data Lengthand pointer to the L-SDU buffer in parameter PduInfoPtr.*/
                                    ...
                                    CANMessageGet(CanContainer.CanConfigSet.CanHardwareObject[index6].
...
/*                                    UARTprintf("Msg ID=0x%08X len=%u data=0x",
                                           ...
                                           UARTprintf("%02X \n",received_CANMessage.pui8MsgData[0]);
                                            ...
                                            Rx_Mailbox.controllerlId =CanContainer.CanConfigSet.CanHardwareObject[index6]
    ...
    while(1)
    {
        ...
        status&=0x80U;
                if(status==CAN_STS_BOFF)
                {
                    ...
                    for(index19=0U;index19<NUM_OF_HOH;index19++)
                                  {
                                      if(CanContainer.CanConfigSet.CanHardwareObject[index19].CanControllerRef== &CanContainer.CanConfigSet
                                              .CanController[0U])
                                      {
                                          if(CanContainer.CanConfigSet.CanHardwareObject[index19].CanObjectType==transmit)
                                          {
                                      for(index20=0U;index20<CanContainer.CanConfigSet.CanHardwareObject[index19].CanHwObjectCount;index20++)
                                      {
                                      CAN0_IF1MCTL_R&=((uint32_t)(~(uint32_t)0x100U));
                                      CAN0_IF1CMSK_R &= ((uint32_t)(~(uint32_t)0x80U));
                                      CAN0_IF1CRQ_R =((uint32_t)((uint32_t) Message_Confirmation[index19][index20].mail_box));
                                      }
                                     }else{/*MISRA*/}
                                   }else{/*MISRA*/}
                                 }
                ...
                status&=0x80U;
                       ...
                       for(index19=0U;index19<NUM_OF_HOH;index19++)
                 {
                     ...
                     CAN1_IF1CMSK_R &= ((uint32_t)(~(uint32_t)0x80U));
                     CAN1_IF1CRQ_R =((uint32_t)((uint32_t) Message_Confirmation[index19][index20].mail_box));
                     }
                    }else{/*MISRA*/}
                  }else{/*MISRA*/}
                }
                   ...
                   if(hohobj->CanTriggerTransmitEnable==FALSE)
                   {
                    ...
                    else
                    {
                      ...
                      for(i=0U;i<hohobj->CanHwObjectCount;i++)/*here i cancel all my transmission requests for this HOH to
                       ...
                       {
                          if(CanContainer.CanConfigSet.CanHardwareObject[i].CanControllerRef== &CanContainer.CanConfigSet
                                                                                        .CanController[0])
                         {
                          CAN0_IF1MCTL_R&=((uint32_t)(~(uint32_t)0x100U));
                          CAN0_IF1CMSK_R&=((uint32_t)(~(uint32_t)0x80U));
...
#if((CanTxProcessing0==INTERRUPT || CanRxProcessing0==INTERRUPT||CanBusoffProcessing0==INTERRUPT)||(CanTxProcessing1==INTERRUPT || CanRxProcessing1==INTERRUPT || CanBusoffProcessing1==INTERRUPT))
             ...
             CanDevolpmentErrorType= CAN_E_PARAM_CONTROLLER;
              ...
              if(IntEnableCount[Controller] >= 1U)
              {
                ...
                {   /*The function Can_DisableControllerInterrupts shall access the CAN controller registers to disable all interrupts
                      ...
                      /*reseting the flag*/
#if(CanTxProcessing0==INTERRUPT || CanRxProcessing0==INTERRUPT || CanBusoffProcessing0==INTERRUPT)
                 if(Controller == 0U)
                 {
...
#if(CanTxProcessing1==INTERRUPT || CanRxProcessing1==INTERRUPT || CanBusoffProcessing1==INTERRUPT)
                  if(Controller == 1U)
                 {
                     /* set interrupt enable, status interrupt enable, error interrupt enable to disable interrupts."*/
   ...
   if(CanUnitState==CAN_UNINIT)
   {
       CanDevolpmentErrorType=CAN_E_UNINIT;
            ...
            if(Controller >= NUM_OF_CAN_CONTROLLERS)
            {
               ...
               if(Int_disable_in_stop_mode[Controller]==0U)/*no need to enable interrupt in stopped mode*/
              {
                  if(IntDisableCount[Controller]==0U)
                  {
#if!(CanTxProcessing0==POLLING&&CanRxProcessing0==POLLING&&CanBusoffProcessing0==POLLING)
                    if(Controller == 0U)
                    {
                    /* set interrupt enable, status interrupt enable, error interrupt enable to disable interrupts.*/
#if!(CanTxProcessing0==INTERRUPT&&CanRxProcessing0==INTERRUPT&&CanBusoffProcessing0==INTERRUPT)
#if(CanTxProcessing0==INTERRUPT&&CanRxProcessing0==INTERRUPT)
                                      CANIntEnable(CAN0_BASE, CAN_INT_MASTER | CAN_INT_STATUS);
#endif
#if CanBusoffProcessing0==INTERRUPT
                                      /*CANIntEnable(CAN0_BASE, CAN_INT_MASTER | CAN_INT_ERROR);*/
                                      /*This case can not be done due to hardware has an error as this
                                     line of code enables interrupt only at bus-off event by setting the CAN_INT_ERROR
                                     flag in CTL register but on testing it generated an interrupt on
                                      receiving and transmitting(hardware seems to have an error*/
#endif
#endif
...
#endif
#if!(CanTxProcessing1==POLLING && CanRxProcessing1==POLLING && CanBusoffProcessing1==POLLING)
                     if(Controller == 1U)
                    {
                        /* set interrupt enable, status interrupt enable, error interrupt enable to disable interrupts."*/

#if!(CanTxProcessing1==INTERRUPT&&CanRxProcessing1==INTERRUPT&&CanBusoffProcessing1==INTERRUPT)
#if(CanTxProcessing1==INTERRUPT&&CanRxProcessing1==INTERRUPT)
                  ...
                  if(((Transition == CAN_T_START) && ( Can_ControllerMode [Controller]!=CAN_CS_STOPPED))||
                   ...
                   hardware registers in a way that makes the CAN controller participating on the network.*/
...
#if CanBusoffProcessing0==INTERRUPT
                                      /*  CANIntEnable(CAN0_BASE, CAN_INT_MASTER | CAN_INT_ERROR);*/
                                    /*This case can not de done due to hardware has an error as this
                                     line of code enables interrupt only at bus-off event by setting the CAN_INT_ERROR
                                     flag in CTL register but on testing it generated an interrupt on
                                      receiving and transmitting(hardware seems to have an error*/
                      ...
                      if(Controller==0U)
                      {
                          ...
                          {
                              Register_Check1=((uint8_t)((uint8_t)CAN0_CTL_R));
                              Register_Check1&=1U;
                             if( Register_Check1==0U)
                             {
                                 break;
                             }else{/*MISRA*/}
                          ...
                          Timer0A_Init();
                              while ((TIMER0_RIS_R & 0x1) == 0U)
                              {
                                  Register_Check1=((uint8_t)((uint8_t)CAN1_CTL_R));
                                  Register_Check1&=1U;
                                 if( Register_Check1==0U)
                                 {
                                     break;
                    ...
                    if(Controller == 0U)
                    {
                        if(IntDisableCount[0U]>=1U)
                            ...
                            by function Can_disableControllerInterrupts.*/
                           {
                            ...
                            {
                                if(CanContainer.CanConfigSet.CanHardwareObject[index9].CanControllerRef== &CanContainer.CanConfigSet
                                        .CanController[0U])
                                {
                                    if(CanContainer.CanConfigSet.CanHardwareObject[index9].CanObjectType==transmit)
                                    {
                                for(index10=0U;index10<CanContainer.CanConfigSet.CanHardwareObject[index9].CanHwObjectCount;index10++)
                                {
                                CAN0_IF1MCTL_R&=((uint32_t)(~(uint32_t)0x100U));
                                CAN0_IF1CMSK_R &= ((uint32_t)(~(uint32_t)0x80U));
                    ...
                    else if(Controller == 1U)
                    {
                        if(IntDisableCount[1U]>=1U)
...
#if!(CanTxProcessing1==INTERRUPT&&CanRxProcessing1==INTERRUPT&&CanBusoffProcessing1==INTERRUPT)
#if(CanTxProcessing1==INTERRUPT&&CanRxProcessing1==INTERRUPT)
                                      CANIntDisable(CAN1_BASE, CAN_INT_MASTER | CAN_INT_STATUS);
#endif
#if CanBusoffProcessing1==INTERRUPT

                                      /*   CANIntDisable(CAN1_BASE, CAN_INT_MASTER | CAN_INT_ERROR);  */
                                     /*This case can not be done due to hardware has an error as this
                                      line of code enables interrupt only at bus-off event by setting the CAN_INT_ERROR
                                      flag in CTL register but on testing it generated an interrupt on
                                       receiving and transmitting(hardware seems to have an error*/
                                 ...
                                 for(index9=0U;index9<NUM_OF_HOH;index9++)
                              {
                                if(CanContainer.CanConfigSet.CanHardwareObject[index9].CanControllerRef==
...
void Can0_InterruptHandler(void) {
            ...
            status0= CANStatusGet(CAN0_BASE, CAN_STS_CONTROL);
                        status0&=0x80U;
                          if(status0==CAN_STS_BOFF)
                          {
                            /*[SWS_Can_00020]  triggered by hardware if the CAN controller reaches bus-off state
                             ...
                             {
                                 if(CanContainer.CanConfigSet.CanHardwareObject[index12].CanControllerRef== &CanContainer.CanConfigSet
                                         .CanController[0U])
                                 {
                                     if(CanContainer.CanConfigSet.CanHardwareObject[index12].CanObjectType==transmit)
                                     {
                                 for(index11=0U;index11<CanContainer.CanConfigSet.CanHardwareObject[index12].CanHwObjectCount;index11++)
                                 {
                                 CAN0_IF1MCTL_R&=((uint32_t)(~(uint32_t)0x100U));
                                 CAN0_IF1CMSK_R &= ((uint32_t)(~(uint32_t)0x80U));
                                 CAN0_IF1CRQ_R =((uint32_t)((uint32_t) Message_Confirmation[index12][index11].mail_box));
                                 }
                                }else{/*MISRA*/}
                              }else{/*MISRA*/}
                             }
                        /*CanIf_ControllerBusOff(0U)*/
                         ...
                         /* [SWS_Can_00273] After bus-off detection, the Can module shall cancel still pending messages.*/
                          /*This is done by Can_SetControllerMode(0U, CAN_T_STOP)*/
    ...
    for(index12=0U;index12< NUM_OF_HOH;index12++)
            {
                if(CanContainer.CanConfigSet.CanHardwareObject[index12].CanControllerRef==&CanContainer
                                                                      .CanConfigSet.CanController[0])
                {
                          if(CanContainer.CanConfigSet.CanHardwareObject[index12].CanObjectType==transmit)
                          {
                                for(index11=0U;index11<CanContainer.CanConfigSet.CanHardwareObject[index12].CanHwObjectCount;index11++)
                                {
                                    ...
                                    if(Message_Confirmation[index12][index11].mail_box==ui32Status0)
                                    {
                                       ...
                                       /* CanIf_TxConfirmation(Message_Confirmation[index12][index11].PduId)*/
                                    }else{}
                              }
                          }
                }
                                ...
                                /*interrupt handling should be deferred*/
                                      ...
                                      the function Can_MainFunction_Read in case of polling mode shall call the callback function
                                       CanIf_RxIndication.*/
                                            ...
                                            CANMessageGet(CanContainer.CanConfigSet.CanHardwareObject[index12].
                                                          CanControllerRef->CanControllerBaseAddress,
                                                    ...
                                                    Rx_Mailbox0.controllerlId =CanContainer.CanConfigSet.CanHardwareObject[index12]
                                                                                             .CanControllerRef->CanControllerId;
...
void Can1_InterruptHandler(void) {
        ...
        if(ui32Status1 == CAN_INT_INTID_STATUS)
        {
            ...
            status1= CANStatusGet(CAN1_BASE, CAN_STS_CONTROL);
                      ...
                      {
                          if(CanContainer.CanConfigSet.CanHardwareObject[index13].CanControllerRef== &CanContainer.CanConfigSet
                                  .CanController[1U])
                          {
                              if(CanContainer.CanConfigSet.CanHardwareObject[index13].CanObjectType==transmit)
                              {
                          for(index14=0U;index14<CanContainer.CanConfigSet.CanHardwareObject[index13].CanHwObjectCount;index14++)
                          {
                          CAN1_IF1MCTL_R&=((uint32_t)(~(uint32_t)0x100U));
                          CAN1_IF1CMSK_R &= ((uint32_t)(~(uint32_t)0x80U));
                          CAN1_IF1CRQ_R =((uint32_t)((uint32_t) Message_Confirmation[index13][index14].mail_box));
                          }
                         }else{/*MISRA*/}
                       }else{/*MISRA*/}
    ...
    for(index13=0U;index13< NUM_OF_HOH;index13++)
            {
                if(CanContainer.CanConfigSet.CanHardwareObject[index13].CanControllerRef==&CanContainer
                                ...
                                /*interrupt handling should be deferred*/
                                      ...
                                      the function Can_MainFunction_Read in case of polling mode shall call the callback function
                                       CanIf_RxIndication.*/
                                            ...
                                            CANMessageGet(CanContainer.CanConfigSet.CanHardwareObject[index13].
                                                          CanControllerRef->CanControllerBaseAddress,
                                                    ...
                                                    Rx_Mailbox1.controllerlId =CanContainer.CanConfigSet.CanHardwareObject[index13]
                                                                                             .CanControllerRef->CanControllerId;
...
void Can_Init(Can_ConfigType* Config)
{
            ...
            CAN_READY, after initialising all controllers inside the HW Unit.*/
    {
        ...
        if  ((Can_ControllerMode [0U]== CAN_CS_UNINIT) && ( Can_ControllerMode[1U]== CAN_CS_UNINIT))
            {
                ...
                {      /*[SWS_Can_00245]  The function Can_Init shall initialise all CAN controllers
                   ...
                   if(Config->CanConfigSet.CanController[index15].CanControllerActivation)
                    {
                    ...
                    /*VIP related to unsent objects*/
                          ...
                          Bit_Time_Parameters0.ui32SyncPropPhase1Seg=(Config->CanConfigSet.CanController[index15].
                                    CanControllerDefaultBaudrate->CanControllerSeg1) + (Config->CanConfigSet.CanController[index15].
                                            CanControllerDefaultBaudrate->CanControllerPropSeg);
                              Bit_Time_Parameters0.ui32Phase2Seg=Config->CanConfigSet.CanController[index15].
                                      CanControllerDefaultBaudrate->CanControllerSeg2;
                              Bit_Time_Parameters0.ui32SJW=Config->CanConfigSet.CanController[index15].
                              ...
                              CANBitTimingSet(Config->CanConfigSet.CanController[index15].CanControllerBaseAddress,
                                                         &Bit_Time_Parameters0);
                              CANDisable(Config->CanConfigSet.CanController[index15].CanControllerBaseAddress);
                    }else{/*MISRA*/}
                }
                ...
                /*interrupt registeration here is fault but it is done here as it shall be done with another module but we donot support it*/
                CANIntRegister(Config->CanConfigSet.CanController[0].CanControllerBaseAddress,Can0_InterruptHandler);

               CANIntRegister(Config->CanConfigSet.CanController[1].CanControllerBaseAddress,Can1_InterruptHandler);
            ...
            /* which means that you have made your module ready , used soon as check*/
        {
            ...
            if(Config->CanConfigSet.CanController[index15].CanControllerActivation)
              {
                Can_ControllerMode [index15]=CAN_CS_STOPPED;
              }else{/*MISRA*/}
...
void Can_DeInit(void)
{
                                       ...
                                       driver is not in state CAN_READY.(SRS_BSW_00369) */
    {
      ...
      /*  Caveat: Caller of the Can_DeInit function has to be sure no CAN controller is in the state STARTED*/
        ...
        {
            if(CanContainer.CanConfigSet.CanController[index17].CanControllerActivation)
            {
        ...
        for(index17 = 0U; index17<NUM_OF_CAN_CONTROLLERS; index17++)
        {
            if(CanContainer.CanConfigSet.CanController[index17].CanControllerActivation)
                        {
                Can_ControllerMode [index17]= CAN_CS_UNINIT;
                        }else{/*MISRA*/}
